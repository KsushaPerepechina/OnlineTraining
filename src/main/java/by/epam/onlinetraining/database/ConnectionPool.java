...
public class ConnectionPool {
    ...
    public ProxyConnection getTransactionalConnection() {
        ProxyConnection connection = getConnection();
        try {
            connection.setAutoCommit(false);
        } catch (SQLException e) {
            LOGGER.error("Can't disable auto commit.", e);
        }
        return connection;
    }
    ...
    public void closeTransactionalConnection(ProxyConnection connection) {
        closeConnection(connection);
        try {
            connection.setAutoCommit(true);
        } catch (SQLException e) {
            LOGGER.error("Can't enable auto commit.", e);
        }
    }
